	...
	switch (framesize){
        ...
        case FRAMESIZE_224X224:
            if( (ret = ov7670_write_array(sensor, ov7670_qvga)) == 0 ) {
                // 320 - 224 = 96
                // 240 - 224 = 16
                // 12 and 150 were determined experimentally
                ret = ov7670_frame_control(sensor, 150 + 96, 150 + 96 + (224 * 2), 12 + (16 * 4), 12 + (224 * 2));
            }
        break;
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t val) {
    return SCCB_Write(sensor->slv_addr, COM9, COM9_SET_AGC(SCCB_Read(sensor->slv_addr, COM9), (uint8_t) val));
}
